    ...
    <el-container class="container-narrow">
      ...
      <el-row class="masthead">
        ...
        <div class="logo-title ">
          <h2 class="muted"><img src="static/logo/b_64.png" style="width:50px;height:50px;margin-bottom:-10px;" alt="">{{$t('menu_api_management')}}
          </h2>
        </div>
        <div class="header-btn-group pull-right">
          <router-link to="/">&nbsp;&nbsp;&nbsp;{{$t('open_platform_name')}}</router-link>
          <router-link v-if="$admin()" to="/admin/index">&nbsp;&nbsp;&nbsp;{{$t('background')}}</router-link>
          &nbsp;&nbsp;&nbsp;
        ...
        <ul class="thumbnails" id="item-list" v-if="itemList">
          ...
          <li class=" text-center">
            <router-link class="thumbnail item-thumbnail"
                         :to="'/' +  (openApiItem.item_domain ? openApiItem.item_domain:openApiItem.item_id )" title="">
              <p class="my-item">{{openApiItem.item_name}}</p>
              <p style="color:red" class="item-description">{{openApiItem.item_description}}</p>
            </router-link>
          </li>

          <li class=" text-center" v-for="item in itemList">
            <router-link class="thumbnail item-thumbnail"
                         :to="'/' +  (item.item_domain ? item.item_domain:item.item_id )" title="">
                  <span class="item-setting " @click.prevent="click_item_setting(item.item_id)"
                        :title="$t('item_setting')">
                    <i class="el-icon-setting"></i>
                  </span>
              <span class="item-top " @click.prevent="click_item_top(item.item_id,item.top)"
                    :title="item.top ? $t('cancel_item_top'):$t('item_top')">
...
<script>
  ...
  export default {
    ...
    methods: {
      get_open_api_item() {
        var that = this;
        var url = DocConfig.server + '/api/item/getOpenApi';

        that.axios.get(url)
          .then(function (response) {
            if (response.data.error_code === 0) {
              var json = response.data.data;
              that.openApiItem = json;
            } else {
